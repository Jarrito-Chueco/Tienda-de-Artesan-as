...
//var_dump($data); // Añade esto para verificar qué datos llegan
//exit(); // Detener el script para no continuar
...
if (!empty($data["nombre"]) && !empty($data["apellido"]) && !empty($data["correo"]) && !empty($data["contrasena"])) {
    ...
    if (preg_match('/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^\w\d\s:])([^\s]){8,}$/', $contrasena)) {
        ...
        if ($sql->fetch_object()) {
            ...
            $sqlUsuario = "INSERT INTO usuario (nombre, correo, contrasena, habilitado, codigo, tipo) 
                           VALUES ('$nombre', '$correo', '$contra', '$habilitado', '$codigo', '$tipo')";

            if ($conn->query($sqlUsuario) === TRUE) {
                $userId = $conn->insert_id;

                $sqlDelivery = "INSERT INTO delivery (apellido, IdUsuario) VALUES ('$apellidos', $userId)";
                if ($conn->query($sqlDelivery) === TRUE) {
                    ...
                    echo json_encode(["success" => false, "message" => "Error al insertar en la tabla delivery: " . $conn->error]);
                }
            } else {
